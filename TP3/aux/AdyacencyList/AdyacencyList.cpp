...
vector <vector < int > >& AdyacencyList::listToMatrix(vector <vector < int > >& costo ){
	
	for(int i=0; i<this->settedEdges;i++){
		/*if((*edges)[i].weigth!=0){
			costo[(*edges)[i].node1,(*edges)[i].node2]= (*edges)[i].weigth;
			costo[(*edges)[i].node2,(*edges)[i].node1]= (*edges)[i].weigth;
		}else{
